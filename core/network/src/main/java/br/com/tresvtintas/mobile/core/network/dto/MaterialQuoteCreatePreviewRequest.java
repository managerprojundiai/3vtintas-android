...
public record MaterialQuoteCreatePreviewRequest(
        long customerId,
        String title,
        String notes,
        String validUntil,
    ...
    public MaterialQuoteCreatePreviewRequest {
        ...
        if (items == null || items.isEmpty() || items.size() > 100
                || items.contains(null)) {
            throw new IllegalArgumentException("Quote preview items are invalid.");
        }
