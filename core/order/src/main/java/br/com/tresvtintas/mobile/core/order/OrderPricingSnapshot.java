...
public record OrderPricingSnapshot(
    ...
    public OrderPricingSnapshot {
        ...
        boolean complete = priceListCode.isPresent()
                && priceListName.isPresent()
                && priceListVersionPublicId.isPresent()
                && priceListVersionNumber.isPresent()
                && policyRevision.isPresent()
                && selectionMode.isPresent()
                && resolvedAt.isPresent();
        if (resolved != complete) {
            throw new IllegalArgumentException("Order pricing snapshot is inconsistent.");
        }
    }
