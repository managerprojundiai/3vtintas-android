package br.com.tresvtintas.mobile.core.whatsappadmin;

import java.util.Objects;
import java.util.Optional;

public final class WhatsAppAdministrationException extends Exception {
    private static final long serialVersionUID = 1L;
    private final WhatsAppAdministrationFailureKind kind;
    private final String requestId;

    public WhatsAppAdministrationException(
            WhatsAppAdministrationFailureKind kind,
            String message) {
        this(kind, message, Optional.empty(), null);
    }

    public WhatsAppAdministrationException(
            WhatsAppAdministrationFailureKind kind,
            String message,
            Throwable cause) {
        this(kind, message, Optional.empty(), cause);
    }

    public WhatsAppAdministrationException(
            WhatsAppAdministrationFailureKind kind,
            String message,
            Optional<String> requestId,
            Throwable cause) {
        super(message, cause);
        this.kind = Objects.requireNonNull(kind, "Failure kind is required.");
        this.requestId = requestId == null ? null : requestId.orElse(null);
    }

    public WhatsAppAdministrationFailureKind kind() {
        return kind;
    }

    public Optional<String> requestId() {
        return Optional.ofNullable(requestId);
    }
}
